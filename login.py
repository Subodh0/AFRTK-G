...
from admin_login import Login_window_admin
from student_main import Face_Recognition_System_student
from email_generator import Email_generate
...
class Login_window:
    def __init__(self,root):
        ...
        #Admin Button
        admin_secret=Label(self.root,text="Admin's Secret Pin",font=("times new roman",13),bg="black",fg="white")
        admin_secret.place(x=850,y=40)

        # self.var_admin=StringVar()
        self.secret_entry=ttk.Entry(self.root,show="*",width=25,font=("times new roman",11))
        self.secret_entry.place(x=1000,y=40)

        #match Button
        self.match_btn=Button(self.root,text="Match",command=self.admin_login,font=("times new roman",12),bd=3,relief=RIDGE,fg="white",bg="blue",activeforeground="white",activebackground="blue")
        self.match_btn.place(x=1200,y=38,width=50,height=30)


        #dev Button
        dev_secret=Label(self.root,text="Developer",font=("times new roman",13),bg="black",fg="white")
        dev_secret.place(x=930,y=650)

        # self.var_admin=StringVar()
        self.dev_entry=ttk.Entry(self.root,show="*",width=25,font=("times new roman",11))
        self.dev_entry.place(x=1010,y=650)

        #match Button
        self.ok_btn=Button(self.root,text="Start",command=self.dev_login,font=("times new roman",12),bd=3,relief=RIDGE,fg="white",bg="blue",activeforeground="white",activebackground="blue")
        self.ok_btn.place(x=1200,y=648,width=50,height=30)
        ...
        #get started
        gt_str=Label(frame,text="Get Started",font=("times new roman",21,"bold"),bg="black",fg="white")
    ...
    def dev_login(self):
        if not self.dev_entry.get()=="face_recognition789123@Ok0#":
            messagebox.showerror("please provide a passcode for dev mode")
        else:
            self.newWindow = Toplevel(self.root)
            self.app = Email_generate(self.newWindow)
    ...
    def admin_login(self):
        if self.secret_entry.get()=="":
            messagebox.showerror("Error","It cannot be empty")
        else:
            conn=mysql.connector.connect(host="localhost",username="root",password="",database="face_recognition")
            my_cursor=conn.cursor()
            # my_cursor.execute("select * from preregister where email_id=%s",(
            #                                                                     self.secret_entry.get()

            #                                                                         ))

            query=("select * from preregister where personal_key=%s")
            ...
            if row==None:
                ...
                self.app = Login_window_admin(self.newWindow)
    ...
    def login(self):
        ...
        if self.var_user.get()=="":
            ...
            conn=mysql.connector.connect(host="localhost",username="root",password="",database="face_recognition")
            my_cursor=conn.cursor()
            my_cursor.execute("select * from student_register where email=%s and password=%s",(
                                                                                        self.var_user.get(),
            ...
            if row==None:
                ...
                self.app = Face_Recognition_System_student(self.newWindow)
                # if open_main>0:
    ...
    def reset_pass(self):
        if self.sec_ques_combo.get()=="Select Security Question":
            ...
            conn=mysql.connector.connect(host="localhost",username="root",password="",database="face_recognition")
            my_cursor=conn.cursor()
            query=("select * from student_register where email=%s and sec_ques=%s and sec_ans=%s")
            ...
            if row==None:
                ...
                Query=("update student_register set password=%s where email=%s")
